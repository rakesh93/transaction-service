...
import com.ilearn.transaction_service.client_call.AccountServiceClient;
...
public class TransactionService {
	...
	public ApiResponse deposit(String accountNumber, TransactionModel transactionModel) {
		...
		if (transactionModel.getAmount() < 0) {
			...
			return new ApiResponse(AppConstants.FAILURE, AppConstants.AMOUNT_GREATER_THAN_ZERO,
	...
	public ApiResponse transfer(TransactionModel transactionModel) {
		...
		if (fromAccountModel == null) {
			logger.error("Sender Account not found with this accountNumber {}",
		...
		if (toAccountModel == null) {
			logger.error("Receiver Account not found with this accountNumber {}",
					transactionModel.getToAccountNumber());
			return new ApiResponse(AppConstants.NOT_FOUND, AppConstants.ACCOUNT_NOT_FOUND, Collections.emptyList());
		}
		...
		// Checking From Account Balance is Greater than Transfer Amount and minimum
		// Balance keep 500
	...
	public AccountResponse getAllAccount() {
		return accountServiceClient.getAllAccount();
	}
	
	public AccountResponse accountServiceFallback(Throwable throwable) {
		return new AccountResponse(AppConstants.SERVER_UNAVAILABLE, AppConstants.ACCOUNT_SERVICE_DOWN, Collections.emptyList());
    }
